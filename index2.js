...
client.on('messageCreate', async message => {
    if (message.content.startsWith('!count')) {
        ...
        // Filter out bots and applications
        const humanMembers = guild.members.cache.filter(member => !member.user.bot);
        const totalMemberCount = humanMembers.size;

        let response = `Total members (excluding bots): **${totalMemberCount}**\n\n`;
        ...
        for (let i = 0; i < countRoleIds.length; i++) {
            ...
            if (hasIgnoredRole) {
                ...
                response += ` (including one ${ignoredRole ? ignoredRole.name : 'ignored role'})`;
            }
            response += '.\n';
        }
